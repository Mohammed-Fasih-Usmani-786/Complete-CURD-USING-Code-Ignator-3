...
class AdminCon extends CI_Controller
{
    ...
    public function updateproduct($id)
    {
        ...
        $this->load->helper(array('form', 'url'));
        if (isset($_POST['save'])) {
            $dd = $this->medoo->select("products", "*", ["row_id" => $id]);




            $iarr = json_decode($dd[0]['_img'], true);
            $imgarr = [];
            $row_id = $this->_crd();
            ...
            if ($this->upload->do_upload('img1')) {
                $data = array('upload_data' => $this->upload->data('file_name'));
                $imgarr[0] = $data['upload_data'];
            } else {
                $imgarr[0] = $iarr[0];
            }
            if ($this->upload->do_upload('img2')) {
                $data = array('upload_data' => $this->upload->data('file_name'));
                $imgarr[1] = $data['upload_data'];
            } else {
                !empty($iarr[1]) ? $imgarr[1] = $iarr[1] : false;
            }
            if ($this->upload->do_upload('img3')) {
                $data = array('upload_data' => $this->upload->data('file_name'));
                $imgarr[2] = $data['upload_data'];
            } else {
                !empty($iarr[2]) ? $imgarr[2] = $iarr[2] : false;
            }

            $_banner = $dd[0]['_banner'];
            if ($this->upload->do_upload('_banner')) {
                $data = array('upload_data' => $this->upload->data('file_name'));
                $_banner = $data['upload_data'];
            }
            ...
            $arr = [
                ...
                "_desr" => $_POST['_desr'],
                "_price" => $_POST['_price'],
                "_techdata" => $_POST['_techdata'],
                "_feature" => $_POST['_feature'],
                "servicecate" => $_POST['servicecate'],
                "_subtitle" => $_POST['_subtitle'],
                "_img" => json_encode($imgarr, true),
            ];

            if (!empty($imgarr)) {
                $x = $this->medoo->update("products", $arr, ["row_id" => $id]);
        ...
        $res = [
            "user" => $this->medoo->select("products", "*", ["row_id" => $id]),
            "m" => $m,
